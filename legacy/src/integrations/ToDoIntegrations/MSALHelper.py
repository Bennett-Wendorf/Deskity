...
def Setup_Msal(to_do_widget_instance, *kwargs):
    ...
    app = PublicClientApplication(settings.To_Do_Widget.app_id, authority="https://login.microsoftonline.com/common", token_cache=cache)
...
def Aquire_Auth_Code(settings):
    ...
    aadAuth = OAuth2Session(settings.To_Do_Widget.app_id, scope=(scopes + ['offline_access']), redirect_uri=redirect_uri)
